...
| 模块 | 核心能力 | 解决的问题 |
|---|---|---|
| **自研行编辑器 (`LineEditor`)** | 纯手写终端输入处理，支持历史记录 `↑`/`↓`、`Tab` 补全（**按常用度排序**）、光标移动及 Emacs 快捷键（`Ctrl+A/E/U/K/W/L`） | 摆脱宿主 shell 的 readline 限制，杜绝 PowerShell ANSI 回显污染 |
| **常用度补全 (`shell/usage` + `shell/complete`)** | 记录手敲过的命令名与次数，跨会话持久化到 `~/.ai-shell/usage.json`；`Tab` 时候选按常用度排序，空输入直接推荐高频命令；多候选只补到最长公共前缀 | 补全列表按真实使用习惯排，越用越顺手；不再按字母序把冷僻命令顶在最前 |
...
| 快捷键 | 功能说明 |
|---|---|
| `Tab` | 按**常用度**补全：候选来自内置命令 + 系统 PATH（SSH 会话下用历史习惯 + 基础命令表），最常用的排最前；空输入直接列出高频命令 |
| `↑` / `↓` | 在历史命令记录中前翻 / 后翻 |
...
AI 走的是**捕获通道**，拿不到 TTY，所以 `sudo`、`su`、`passwd`、`mysql -p` 这类命令
**无法代你输入密码**。如果放任它跑，就会卡在密码提示上等超时，然后模型还会换着法儿再试
几种 sudo 变体，几轮下来什么都没查到 —— 所以这类命令会在**执行前**就被拦下来，
问你要不要**交回你自己的终端执行**：

```text
这条命令需要你在终端里输密码 (sudo 需要你的登录密码，AI 不能代你输入)
  sudo ufw status verbose
  AI 的捕获通道拿不到 TTY，代跑只会卡在密码提示上。交回你的终端执行，密码提示出来自己输即可。
  想让它以后能自动跑：给该用户配 NOPASSWD（sudo visudo 加一行 `用户名 ALL=(ALL) NOPASSWD:ALL`）。
  y 在本终端执行   N 跳过(默认)   e 改成别的命令   
```

- **按 `y`：命令直接写进你的终端执行**（SSH 会话下等同你手敲——远端是真实 TTY，
  密码提示出来自己输就行；root / NOPASSWD 场景则直接跑完）。这是最常用的一条路。
- **默认 `N` 跳过**，并就此收尾 —— 不会让模型继续瞎试，轮数不会被烧光。
- **按 `e` 改成别的命令**（比如换成免密的只读写法）后再执行。
- 即使 AI 没预判到（模型没写 sudo，但命令照样弹了密码提示），捕获器也会**嗅到提示立即中断**，
  并同样提供「交回终端执行」的选择，不会让你干等，也不会再往下试。
- 模型主动交出 sudo 命令收尾时（`done:true`），命令**一定会被交到确认框里**——
  不会只打一句「需要你手动输入密码」就什么都不干。
- 识别范围（执行前预判）：`sudo`（不含 `sudo -n` / `sudo -S`）、`su`、`passwd`、`ssh-add`、
  `ssh-keygen`、`mysql -p`、`psql -W`、`read -s`。`sudo -S` 这类会读 stdin 的写法属于故意
  绕开提示，落在上面那条**运行时嗅探**兜底里。
- 本地内置 shell 是非交互环境，没有可交回的终端，会把命令完整打印出来请你手敲（命令一定看得见）。
...
MonoShell/
├── src/
│   ├── index.ts          # CLI 入口与参数路由（组合根，只做分发）
│   ├── config.ts         # 配置加载 / 默认值 / 保存（自动 .bak 备份）与安全规则定义
│   ├── app/              # 编排层：把各模块组装成可运行的会话
│   │   ├── repl.ts       # REPL 主循环（触发判定 / 危险确认 / AI 接管）
│   │   ├── local-shell.ts# 本地内置 shell 启动
│   │   ├── ssh-shell.ts  # SSH 会话全生命周期（登录标记握手 / 退出探活 / 提示符时序）
│   │   └── picker.ts     # 连接选择 -> 打开 shell 的循环
│   ├── cli/
...
│   ├── session/
│   │   ├── types.ts      # 会话抽象：SessionContext（环境描述）+ Session（IO 能力）
│   │   ├── local.ts      # 本地终端会话封装 (node:child_process)
│   │   ├── ssh.ts        # SSH2 远程通道与自动配置
│   │   └── handshake.ts  # 远端握手标记判定（裸 \r / ANSI 规范化，纯函数可测）
│   ├── shell/
│   │   ├── line.ts       # 自研终端行编辑器 (输入回显/光标/Tab补全/快捷键)
│   │   ├── complete.ts   # 补全候选来源与按常用度排序 (本地/SSH 两套)
│   │   ├── usage.ts      # 命令使用统计与持久化 (~/.ai-shell/usage.json)
...
│   ├── agent.ts          # AI 策略 / 重试 / 危险命令分级 / 密码命令终端接管
